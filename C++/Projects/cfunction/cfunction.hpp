...
template<class XType,class YType>class c_crdinate{			//用于构建坐标系 
	...
	public:
		c_crdinate(void){		}
		c_crdinate(XType __Give_X,YType __Give_Y){this->__Xpos=__Give_X;this->__Ypos=__Give_Y;}
		...
		c_crdinate get(void){return *this;}
		inline long double distance_between(c_crdinate __other){tmp_betw=sqrt((this->__Xpos-__other.__Xpos)*(this->__Xpos-__other.__Xpos)+(this->__Ypos-__other.__Ypos)*(this->__Ypos-__other.__Ypos));return tmp_betw;}
...
template<class type>class c_list{
	...
	public:
		bool isSet=false;
		c_list(){		}
		c_list(int __size){this->__a=new type[__size+1];isSet=true;this->__SSize=__size;}
...
template<typename _T1,typename _T2>c_dictionary<_T1,_T2> make_dictionary(_T1 __name,_T2 __data){
	c_dictionary<_T1,_T2> __res(1);
	__res.make_word(__name,__data);
	return __res;
}//用于制造字典
template<typename _T1,typename _T2>c_dictionary<_T1,_T2> make_dictionary(_T1 __name,_T2 __data,_T1 __name1,_T2 __data1){
	c_dictionary<_T1,_T2> __res(2);
	__res.make_word(__name,__data);
...
//	将厘米转换英尺
	template<typename T>T sigma(T arr[],int ll,int ul){
		T sum;
		for(int i=ll;i<=ul;i++) sum+=arr[i];
		return sum;
	}
};
